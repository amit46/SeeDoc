...
export default function PhysicianDashboard({ result, chiefComplaint }) {
  const complexityClass = result ? `complexity-badge complexity-${result.complexityScore}` : "";

  return (
    <div>
      {/* ── Calendar always visible ── */}
      <div className="card" style={{ marginBottom: "1.25rem" }}>
        <p className="card-title">📅 Appointment Calendar</p>
        <AppointmentCalendar />
      </div>

      {/* ── Clinical briefing (only when a result exists) ── */}
      {!result ? (
